...
namespace DataDisplay.Scripts
{
    ...
    public class CameraAircraft : MonoBehaviour
    {
        ...
        private float boost = 1f, brake = 1f;
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyUp(KeyCode.Y) && audioSource != null)
            {
            ...
            //Boost or Brake
            if (Input.GetKey(KeyCode.LeftShift)) boost = 2;
            else boost = 1;
            if (Input.GetKey(KeyCode.LeftControl)) brake = .25f;
            else brake = 1;
            //
            ...
            //Lift
            transform.Translate(((Input.GetKey(KeyCode.R) ? 1 : 0) - (Input.GetKey(KeyCode.F) ? 1 : 0)) * Vector3.up
                * Time.deltaTime * (lift * 5) * boost * brake); //Up and Down (Drones+Helicopters)
            //
