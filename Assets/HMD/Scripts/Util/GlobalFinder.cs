...
namespace HMD.Scripts.Util
{
    public static class GlobalFinder
    {
        ...
        public class Found
        {
            [NotNull] public readonly string Query;
            ...
            {
                if (All.Length == 0)
                    throw new Exception("No GameObject with name " + Query + " found in scene.");
            ...
            {
                if (All.Length > 1)
                    throw new Exception($"{All.Length} GameObjects with name " + Query + " found in scene.");
